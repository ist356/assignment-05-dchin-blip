...
web_survey_df["_year"] = web_survey_df["Timestamp"].apply(pl.extract_year_mdy)
years = web_survey_df["_year"].unique()
for year in years:
    web_COL = pd.read_html(f"https://www.numbeo.com/cost-of-living/rankings.jsp?title={year}&displayColumn=0")
    web_COL_df = web_COL[1]
    web_COL_df["_year"] = year
    web_COL_df.to_csv(f"cache/col_{year}.csv")
web_survey_df.to_csv("cache/survey.csv")
...
web_states_df.to_csv("cache/states.csv")
